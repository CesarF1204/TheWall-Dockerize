...
class UsersModel extends DatabaseModel {
    ...
    insertUserAccount = async (params) => {
        ...
        try{
            ...
            /* Check if email address is not taken and has no record on the database */
            if(!status && !result.email_address){
                let inser_user_account_query = mysql.format(`INSERT INTO users SET ?`, users_data);
                let insert_user_account_result = await this.query(inser_user_account_query);
