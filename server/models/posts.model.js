...
class PostsModel extends DatabaseModel {
    ...
    deletePost = async (post_id) => {
        ...
        try{
            if(post_id){
                let delete_post_query = mysql.format(`DELETE FROM posts WHERE id = ?;`, post_id);
                let delete_post_result = await this.query(delete_post_query);

                response_data.result = delete_post_result;
                response_data.status = true;
            }
        }
        catch(error){
